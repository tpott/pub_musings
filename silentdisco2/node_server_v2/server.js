...
async function main() {
  ...
  // TODO when running `node --inspect ../node_server_v2/server.js` from the
  // silentdisco dir, this file isn't in the local path..
  await fs.copyFile(
    path.join(__dirname, 'e_J14fbBluE.mp3'), // __dirname should be $something/node_server_v2/
  ...
  app.use(express.static(path.join(__dirname, '../silentdisco_v2/build')));
  ...
  app.get('/', (req, res) => {
    ...
    res.sendFile(path.join(__dirname, '../silentdisco_v2/build/index.html'));
  });
