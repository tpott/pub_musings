...
class TestFetchFeedback(unittest.TestCase):
    ...
    def test_none_script_does_nothing(self) -> None:
        """When script_path is None, should do nothing."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            log_file = Path(tmp_dir) / "test.log"
...
class TestCountPendingTasks(unittest.TestCase):
    def test_counts_todo_tasks(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            project = Path(tmp_dir) / "myproject"
            project.mkdir()
            tasks = project / "TASKS.jsonl"
            tasks.write_text(
            ...
            os.chdir(tmp_dir)
            try:
                result = count_pending_tasks("myproject")
            finally:
                os.chdir(old_cwd)
            self.assertEqual(result, 2)  # todo + pending

    def test_returns_zero_when_no_tasks_file(self) -> None:
        result = count_pending_tasks("nonexistent_project")
        self.assertEqual(result, 0)


class TestCheckFeedbackWaiting(unittest.TestCase):
    def test_returns_true_when_feedback_exists(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            project = Path(tmp_dir) / "myproject"
            project.mkdir()
            ...
            os.chdir(tmp_dir)
            try:
                result = check_feedback_waiting("myproject")
            finally:
                os.chdir(old_cwd)
            self.assertTrue(result)

    def test_returns_false_when_no_feedback(self) -> None:
        result = check_feedback_waiting("nonexistent_project")
        self.assertFalse(result)


class TestBuildPrompt(unittest.TestCase):
    def test_includes_template_and_project_table(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            # Create RALPH.md template
            ralph_dir = Path(tmp_dir) / "ralph"
            ralph_dir.mkdir()
            (ralph_dir / "RALPH.md").write_text("# Template\nHello Ralph.\n")

            # Create project dir with tasks
            ...
            config = RalphConfig(
                projects={
                    "testproj": ProjectConfig(
                        name="testproj",
                        feedback_script="testproj/scripts/fetch-feedback.py",
    ...
    def test_project_filter(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            ralph_mod.PROMPT_FILE = Path("ralph/RALPH.md")
            try:
                result = build_prompt(config, project_filter="projA")
            finally:
                os.chdir(old_cwd)
                ralph_mod.PROMPT_FILE = old_prompt
