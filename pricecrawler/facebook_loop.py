...
async def get_my_id(page_token: str) -> Optional[str]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://graph.facebook.com/me?metadata=true&access_token={page_token}') as resp:
...
async def check_token_expiry(page_token: str, app_id: str, app_secret: str) -> int:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://graph.facebook.com/debug_token?input_token={page_token}&access_token={app_id}|{app_secret}') as resp:
            results = await resp.json()
            if 'error' in results:
                print(results['error'])
                return 0
...
async def maybe_refresh_to_non_expiring_token(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id={app_id}&client_secret={app_secret}&fb_exchange_token={page_token}') as resp:
            results = await resp.json()
            if 'error' in results:
                print(results['error'])
                return page_token
...
async def get_recent_conversations(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://graph.facebook.com/{page_id}/conversations?fields=participants,updated_time&access_token={page_token}') as resp:
            results = await resp.json()
            print(results)
            filtered = []
...
async def get_recent_messages(conversation_id: str, page_token: str) -> List[Dict[str, Any]]:
    """Get recent messages from a conversation."""
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://graph.facebook.com/{conversation_id}?fields=messages{{message,created_time,from}}&access_token={page_token}') as resp:
            results = await resp.json()
            print(results)
...
async def post_message(
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f'https://graph.facebook.com/{page_id}/messages', json=params) as result:
            print(result)
            if 400 <= result.status and result.status < 600:
                print(f'{result.status} error post to graph.facebook.com/{page_id}/messages')
...
async def run_once(token_expiry_cache: Optional[Dict[str, int]] = None) -> None:
    ...
    for page_config in config['pages']:
        ...
        new_token = await maybe_refresh_to_non_expiring_token(app_id, app_secret, page_token, token_expiry_cache)
        ...
        conversations = await get_recent_conversations(page_id, page_token, last_run_time)
        for conv in conversations:
            ...
            if len(unpaid_users) > 0:
                for unpaid_user in unpaid_users:
                    ...
                    await post_message(page_id, page_token, (conversation_id, user_id), f"@{unpaid_user['name']} in order to use price checker, please complete payment at {payment_url}")
                continue
            ...
            if 'error' in summary_obj:
                await post_message(page_id, page_token, (conversation_id, user_id), summary_obj['error'])
                continue
            for summary in summary_obj['summaries']:
                ...
                if 'summary_text' not in summary:
                    await post_message(page_id, page_token, (conversation_id, user_id), f"{target}\nURL: {url}")
                    continue
