...
def check_api_key_file(file_path):
    ...
    if not (
        ...
        and not file_mode & stat.S_IRWXG
        and not file_mode & stat.S_IRWXO
    ):
...
def main():
    ...
    parser.add_argument(
        "--players", type=int, default=7, help="Number of players (default: 7)"
    )
    parser.add_argument(
        "--werewolves", type=int, default=2, help="Number of werewolves (default: 2)"
    )
    parser.add_argument(
        "--seers", type=int, default=1, help="Number of seers (default: 1)"
    )

    # API options - only one should be provided
    api_group = parser.add_mutually_exclusive_group(required=True)
    api_group.add_argument(
        "--openai-key-file", type=str, help="File containing OpenAI API key"
    )
    api_group.add_argument(
        "--anthropic-key-file", type=str, help="File containing Anthropic API key"
    )

    parser.add_argument(
        "--model", type=str, help="Model to use (defaults based on API choice)"
    )
    parser.add_argument(
        "--max-words",
        type=int,
        default=80,
        help="Maximum number of words per AI response (default: 80)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log-to-file",
        action="store_true",
        help="Log detailed prompts and responses to logs.txt",
    )
