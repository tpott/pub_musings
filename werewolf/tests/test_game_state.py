...
class TestNightPhaseTransitions(unittest.TestCase):
    ...
    def test_werewolf_actions_after_seer(self):
        ...
        with patch.object(self.game, '_get_ai_response', side_effect=mock_get_ai_response), \
             patch.object(self.game, '_execute_player_action', wraps=self.game._execute_player_action) as mock_execute:
            ...
            for call_args in mock_execute.call_args_list:
                if len(call_args[0]) >= 1:  # Make sure we have arguments
                    ...
                    if hasattr(player, 'role'):  # Check it's actually a player
    ...
    def test_method_sequence_in_night_phase(self):
        ...
        with patch.object(self.game, '_get_ai_response', side_effect=mock_get_ai_response), \
             patch.object(self.game, '_execute_player_action', side_effect=track_execution), \
             patch.object(self.game.state, 'advance_phase', side_effect=track_advance_phase), \
             patch.object(self.game, '_process_werewolf_kill', return_value=None):
            ...
            self.assertIn('werewolf_action', action_sequence, "Werewolf action was not recorded")
            self.assertIn('seer_action', action_sequence, "Seer action was not recorded")
            ...
            self.assertEqual(action_sequence[-1], 'advance_phase', 
