...
class Game:
    ...
    def _run_day_voting_phase(self) -> None:
        ...
        for player in living_players:
            ...
            vote = "".join(
                c for c in vote if c.isalnum() or c.isspace()
            )  # Remove punctuation
            ...
            if (
                ...
                potential_targets = [
                    p.name for p in living_players if p.name != player.name
                ]
                if potential_targets:
                    random_vote = random.choice(potential_targets)
                    self.state.votes[player.name] = random_vote
                    print(
                        f"{player.name} casts an unclear vote, interpreted as a vote for {random_vote}"
                    )
    ...
    def _process_werewolf_actions(self, werewolves: List[Player]) -> None:
        ...
        for werewolf in werewolves:
            ...
            if (
                target_player
                and target_player.alive
                and target_player.role != Role.WEREWOLF
            ):
        ...
        if werewolf_votes:
            ...
            candidates = [
                name for name, votes in werewolf_votes.items() if votes == max_votes
            ]
    ...
    def _process_seer_action(self, seer: Player) -> None:
        ...
        response = "".join(
            c for c in response if c.isalnum() or c.isspace()
        )  # Remove punctuation
        ...
        if target_player and target_player.alive and target_player.name != seer.name:
            ...
            if self.config.verbose:
                print(
                    f"Seer {seer.name} investigates {target_player.name} and learns they are {result}"
                )
