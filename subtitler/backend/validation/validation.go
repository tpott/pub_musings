...
// MaxSessionIDLength is the maximum length for anonymous session IDs.
const MaxSessionIDLength = 64

// ValidateSessionID validates the session_id query parameter format.
// Returns the session_id unchanged if valid, or empty string if invalid.
// Valid session_ids are 1-64 characters containing only alphanumeric chars and hyphens.
// The frontend generates 32-char hex strings; this validation is intentionally
// permissive to support various ID formats while blocking injection attempts.
