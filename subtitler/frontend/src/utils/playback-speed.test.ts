...
describe('playback-speed utility', () => {
    ...
    describe('PLAYBACK_SPEEDS constant', () => {
        it('should contain only 0.8x, 0.9x, 1.0x', () => {
            expect(PLAYBACK_SPEEDS).toEqual([0.8, 0.9, 1.0]);
        });

        it('should NOT contain 0.5x, 0.75x, 1.25x, 1.5x, or 2x', () => {
            // This test exists to prevent regressions - speeds have been
            // incorrectly expanded multiple times. Only 3 speeds are allowed.
            expect(PLAYBACK_SPEEDS).not.toContain(0.5);
            expect(PLAYBACK_SPEEDS).not.toContain(0.75);
            expect(PLAYBACK_SPEEDS).not.toContain(1.25);
            expect(PLAYBACK_SPEEDS).not.toContain(1.5);
            expect(PLAYBACK_SPEEDS).not.toContain(2.0);
        });

        it('should have exactly 3 speed options', () => {
            expect(PLAYBACK_SPEEDS).toHaveLength(3);
        });
    ...
    describe('getSavedSpeed', () => {
        ...
        it('should return saved value when valid', () => {
            localStorageMock._setStore({ 'subtitler:playback-speed': '0.9' });
            expect(getSavedSpeed()).toBe(0.9);
        });
        ...
        it('should return default for old 6-speed values that are no longer valid', () => {
            // These were previously valid but are now rejected
            localStorageMock._setStore({ 'subtitler:playback-speed': '0.5' });
            expect(getSavedSpeed()).toBe(DEFAULT_SPEED);
            localStorageMock._setStore({ 'subtitler:playback-speed': '0.75' });
            expect(getSavedSpeed()).toBe(DEFAULT_SPEED);
            localStorageMock._setStore({ 'subtitler:playback-speed': '1.25' });
            expect(getSavedSpeed()).toBe(DEFAULT_SPEED);
            localStorageMock._setStore({ 'subtitler:playback-speed': '1.5' });
            expect(getSavedSpeed()).toBe(DEFAULT_SPEED);
            localStorageMock._setStore({ 'subtitler:playback-speed': '2' });
            expect(getSavedSpeed()).toBe(DEFAULT_SPEED);
        });
